Parte do Ruan:
- Mostrar a criação de repositorios e seus tipos
- Mostrar como manipular arquivos e salvar alterações pelo gitkraken
- Mostrar nosso repositorio de exemplo para a explicação de o que é uma baseline
- Indicar que o ramo principal mais conhecido como master também é chamado de trunk("Tronco" do ingles)
- Mostrar como é a criação de branches
- Mostrar os sistemas de check-out e check-in utilizando branches
Parte do Ian:
-Mostrar o Merge de branchs
-Mostrar o uso do programa em tempo real
-Apresentra outras features do programa

testes

Criação de repositorios:
	Existem dois tipos de criação de repositorio, a local e a remota... Quando criamos um repositorio local, os arquivos uteis são somente os que
existem em nosso computador, podemos transforma-lo em um repositorio remote também desde que façamos o push do repositorio local para um remoto.
	Existem alguns tipos de servidores onde podemos armazenar nossos repositorios ou criar um por la mesmo, um deles é o proprio github 
e por meio do gitkraken clona-lo e fazer alterações, apos o termino dessas alterações podemos dar o comando push e faremos o upload dos arquivos locais 
para o remoto.

O termo baseline pode ser caracterizado como a linha base para o acompanhamento de qualquer tipo de projeto. Além de acompanhá-lo,
a referida linha pode ser utilizada para reproduzir e rastrear todas as etapas e todos os acontecimentos delineados dentro do projeto. 
No caso do gitkraken, ele utiliza uma baseline de escopo, mapeando tudo que se produziu e ira ser produzido, seperando tais etapas em suas devidas versões.

O termo trunk se refere ao branch principal do seu repositorio, que no caso seria o nosso branch master, podemos chamar ele de master ou trunk (ou tronco, do ingles) 
pois ele se trata da base de todo o repositorio, onde a versão ja lançada está.
  